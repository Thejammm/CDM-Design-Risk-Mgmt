// ══════════════════════════════════════════════════════════════
//  CDM Rolling Risk Register — server
//
//  - Serves the front-end (PD risk register app) from public/
//  - Postgres-backed per-tenant state (one JSONB blob per tenant)
//  - Email/password auth with JWT in httpOnly cookie
//  - Bootstrap creates a consultant user on first run
//
//  Architecture mirrors the AHS Workplace Inspection System
//  (inspections.archerhs.co.uk) — single Node service serves both
//  the static frontend AND the authenticated API.
// ══════════════════════════════════════════════════════════════
...
(async () => {
  try {
    ...
    app.listen(PORT, HOST, () => {
      console.log(`✓ CDM Risk Register listening on http://${HOST}:${PORT}`);
    });
  } catch(err){
